@extends('master')
@section('title', 'Kelola Data Batch Rekrutmen')
@section('content')
        ...
        <div class="table-responsive mb-4 mt-4">
            <table id="zero-config" class="table table-hover table-striped" style="width:100%">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{$list->position}}</td>
                        <td>{{$months[$list->month]}} {{$list->year}}</td>
                        <td>
							  <a href="/batch/{{$list->id}}" class="btn btn-warning btn-sm">
							  ...
							  <form action="{{route('batch.destroy', $list->id)}}" method="post">
							  	@csrf
